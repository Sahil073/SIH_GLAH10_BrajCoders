...
// Features fully automatic hands-free connection and background auto-reconnect.
// =============================================================================
...
export class BleService {
  ...
  private onPacketListener?: PacketCallback;
  private onStatusListener?: StatusCallback;
  private onDevicesListener?: DeviceDiscoveredCallback;
  private onLogListener?: LogCallback;
  ...
  public setCallbacks(
    ...
    this.onPacketListener = onPacket;
    this.onStatusListener = onStatus;
    this.onDevicesListener = onDevices;
  ...
  public async startAutoConnect(): Promise<void> {
    ...
    if (!this.isNativeBleAvailable) {
      this.log('info', 'Running in Expo Go — starting simulated auto-stream...');
    ...
    if (!hasPerm) {
      this.onStatusListener?.('disconnected', 'Bluetooth permissions required to connect');
      return;
    }
  ...
  public async startScan(): Promise<void> {
    ...
    if (!this.isNativeBleAvailable) {
      this.onStatusListener?.('scanning');
      setTimeout(() => {
        ...
        this.onDevicesListener?.(Array.from(this.discoveredDevicesMap.values()));
    ...
    if (!hasPermission) {
      this.onStatusListener?.('disconnected', 'Bluetooth permissions required');
      return;
    }
          ...
          if (device && (device.name || device.localName)) {
            ...
            const isEsp32 =
              ...
              (device.serviceUUIDs &&
                device.serviceUUIDs.some(
                  (u: string) => u.toLowerCase() === NUS_SERVICE_UUID.toLowerCase()
                ));
            ...
            if (isEsp32) {
              ...
              if (this.isAutoConnectEnabled && !this.isConnectingOrConnected) {
                this.log('info', `Found ${name}! Auto-connecting immediately...`);
  ...
  public async connectToDevice(deviceId: string): Promise<void> {
    ...
    this.onStatusListener?.('connecting');
    ...
    try {
      ...
      device.monitorCharacteristicForService(
        ...
        (error: any, characteristic: any) => {
          ...
          if (characteristic?.value) {
            const rawChunk = StreamPacketParser.decodeBase64(characteristic.value);
      ...
      device.onDisconnected((error: any, disconnectedDevice: any) => {
        ...
        this.streamParser.reset();
        this.onStatusListener?.('disconnected');
        this.scheduleReconnect();
      });

      this.onStatusListener?.('connected');
      this.log('info', 'Sensor Stream Active (Nordic UART Service)');
    } catch (err: any) {
      this.log('error', `Connection failed: ${err?.message || err}`);
      this.connectedDevice = null;
      this.isConnectingOrConnected = false;
      this.onStatusListener?.('disconnected', err?.message || 'Connection failed');
      this.scheduleReconnect();
    }
  }
  ...
  public startSimulation(): void {
    ...
    this.simulationInterval = setInterval(() => {
      ...
      for (let i = 0; i < 128; i++) {
        simAngle += 0.05;
        const qrs = Math.sin(simAngle) > 0.95 ? 650 * Math.sin(simAngle * 10) : 0;
        const noise = (Math.random() - 0.5) * 30;
        const sampleVal = Math.round(baseBaseline + 120 * Math.sin(simAngle) + qrs + noise);
        samples.push(Math.max(0, Math.min(4095, sampleVal)));
      }

      this.onPacketListener?.({
        v: 1,
        sensor: SensorId.EXG,
      ...
      if (this.simSeqCounter % 4 === 0) {
        this.onPacketListener?.({
          ...
          ts: uptime,
          data: {
            temperature: parseFloat((26.5 + Math.sin(simAngle * 0.1) * 1.5).toFixed(1)),
            humidity: parseFloat((58.0 + Math.cos(simAngle * 0.1) * 3.0).toFixed(1)),
          },
        });
      }
